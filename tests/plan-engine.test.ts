...
test("plan validation rejects calendar-invalid dates", () => {
  assert.throws(
    () => buildAdaptivePlan({ courses, availability, fromDate: "2026-02-30" }),
    /计划起始日期无效/,
  );
  assert.throws(
    () =>
      buildAdaptivePlan({
        courses: [{ ...courses[0]!, examDate: "2026-02-30" }],
        availability,
        fromDate: "2026-12-09",
      }),
    /课程考试日期无效/,
...
test("plan phases follow exam proximity instead of one fixed label", () => {
  const far = buildAdaptivePlan({
    courses: [{ ...courses[0]!, examDate: "2026-12-30" }],
    availability: [{ date: "2026-12-09", minutes: 60 }],
    fromDate: "2026-12-09",
  });
  assert.match(far[0]?.title ?? "", /系统梳理/);
  const near = buildAdaptivePlan({
    courses: [{ ...courses[0]!, examDate: "2026-12-11" }],
    availability: [{ date: "2026-12-09", minutes: 60 }],
    fromDate: "2026-12-09",
  });
...
test("consecutive days rotate evidence-backed focuses instead of repeating one", () => {
  ...
  assert.equal(
    new Set(titles).size,
    titles.length,
    "no two tasks may repeat one focus while alternatives remain",
  );
});
  ...
  assert.throws(
    () =>
      buildAdaptivePlan({
        ...
        availability,
        fromDate: "2026-12-09",
        recentMisses: [{ courseId: "urgent", topic: "", missedOn: "2026-12-08" }],
      }),
    /错题主题格式无效/,
  );
  assert.throws(
    () =>
      buildAdaptivePlan({
        ...
        availability,
        fromDate: "2026-12-09",
        recentMisses: [{ courseId: "urgent", topic: "极限", missedOn: "2026-13-01" }],
      }),
    /错题日期无效/,
  );
});
