  ...
  keyPoints: [
    {
      ...
      title: "哈希冲突处理",
      importance: 5,
      evidence: { label: "plan.pdf", location: "第 1 页", quote: "冲突次数统计" },
    },
  ],
  questionPatterns: [],
...
test("plan/generate calls the AI for the persisted plan and honestly falls back without a key", async () => {
  ...
  const provider = createServer(async (request, response) => {
    ...
    if (
      ...
      body.includes("final_exam_study_plan")
    ) {
  ...
  try {
    ...
    const { material } = await storeUploadedMaterial(
      course.id,
      new File([Buffer.from("%PDF-1.4 plan")], "plan.pdf", { type: "application/pdf" }),
    );
    ...
    const guardedResponse = await generatePlan(
      new NextRequest("http://localhost/api/plan/generate", {
        method: "POST",
        headers: {
    ...
    assert.equal(
      ...
      403,
      "a browser endpoint must not receive the server environment key",
    );
    assert.match(String(((await guardedResponse.json()) as { error?: string }).error), /会话 Key/);
    assert.equal(
      providerRequests.length,
      requestsBeforeGuard,
      "credential pairing must be rejected before any provider request",
    );
    ...
    if (previous.serverKeyAccess === undefined)
      delete environment.AI_ALLOW_UNAUTHENTICATED_SERVER_KEY;
    else environment.AI_ALLOW_UNAUTHENTICATED_SERVER_KEY = previous.serverKeyAccess;
    if (previous.customBaseUrl === undefined) delete environment.AI_ALLOW_CUSTOM_BASE_URL;
    else environment.AI_ALLOW_CUSTOM_BASE_URL = previous.customBaseUrl;
    ...
    const aiPayload = (await aiResponse.json()) as {
      ...
      workspace?: {
        planSource?: string;
        tasks: Array<{ date: string; duration: number; courseId: string }>;
        availability: Array<{ date: string; minutes: number }>;
      };
    };
    assert.equal(
      aiPayload.generatedBy,
      "ai",
      "a configured key must produce a real AI-generated plan",
    );
    assert.equal(aiPayload.workspace?.planSource, "ai");
    assert.ok((aiPayload.workspace?.tasks.length ?? 0) > 0);
    const capacity = new Map(
      (aiPayload.workspace?.availability ?? []).map((day) => [day.date, day.minutes]),
    );
    ...
    assert.ok(
      [...capacity.values()].every((minutes) => minutes >= 0),
      "AI plan must never exceed daily capacity",
    );
    ...
    const staleGeneration = generatePlan(
      new NextRequest("http://localhost/api/plan/generate", {
        method: "POST",
        headers: {
          "content-type": "application/json",
    ...
    try {
      ...
      await updateAvailability(
        duringGeneration.availability.map((day, index) => ({
          ...day,
    ...
    assert.equal(
      ...
      409,
      "a plan generated from changed inputs must be rejected at commit time",
    );
    assert.match(
      String(((await staleResponse.json()) as { error?: string }).error),
      /学习数据发生变化/,
    );
    const afterStaleResponse = await getWorkspace();
    assert.equal(
      afterStaleResponse.planSource,
      "schedule",
      "the stale AI result must not replace the newer local replan",
    );
    assert.equal(
      afterStaleResponse.planGenerationLease,
      undefined,
      "a failed commit releases its own reservation",
    );

    const fallbackResponse = await generatePlan(
      new NextRequest("http://localhost/api/plan/generate", {
        method: "POST",
    ...
    const fallbackPayload = (await fallbackResponse.json()) as {
      generatedBy?: string;
      workspace?: { planSource?: string; tasks: Array<{ courseId: string }> };
    };
    assert.equal(
      fallbackPayload.generatedBy,
      "schedule",
      "without a key the deterministic scheduler must be labeled honestly",
    );
    ...
    if (previous.dataDir === undefined) delete environment.FINALE_DATA_DIR;
    else environment.FINALE_DATA_DIR = previous.dataDir;
    if (previous.apiKey === undefined) delete environment.OPENAI_API_KEY;
    else environment.OPENAI_API_KEY = previous.apiKey;
    if (previous.aiKey === undefined) delete environment.AI_API_KEY;
    else environment.AI_API_KEY = previous.aiKey;
    if (previous.nodeEnv === undefined) delete environment.NODE_ENV;
    else environment.NODE_ENV = previous.nodeEnv;
    if (previous.serverKeyAccess === undefined)
      delete environment.AI_ALLOW_UNAUTHENTICATED_SERVER_KEY;
