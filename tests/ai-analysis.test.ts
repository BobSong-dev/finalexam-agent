...
test("custom AI request URLs accept an API root and reject endpoint URLs", () => {
  assert.equal(
    getSessionBaseUrl("https://gateway.example.com/v1/"),
...
test("custom AI URL validation rejects private IPv6 and mapped addresses", () => {
  ...
  try {
    ...
    if (originalCustomBaseUrl === undefined) delete environment.AI_ALLOW_CUSTOM_BASE_URL;
    else environment.AI_ALLOW_CUSTOM_BASE_URL = originalCustomBaseUrl;
  }
});
...
test("a trusted operator endpoint may be private in production while a browser endpoint may not", () => {
  ...
  try {
    ...
    if (originalBaseUrl === undefined) delete environment.OPENAI_BASE_URL;
    else environment.OPENAI_BASE_URL = originalBaseUrl;
    if (originalApiKey === undefined) delete environment.OPENAI_API_KEY;
    else environment.OPENAI_API_KEY = originalApiKey;
    if (originalAiKey === undefined) delete environment.AI_API_KEY;
    else environment.AI_API_KEY = originalAiKey;
    if (originalServerKeyAccess === undefined)
      delete environment.AI_ALLOW_UNAUTHENTICATED_SERVER_KEY;
    else environment.AI_ALLOW_UNAUTHENTICATED_SERVER_KEY = originalServerKeyAccess;
    if (originalCustomBaseUrl === undefined) delete environment.AI_ALLOW_CUSTOM_BASE_URL;
    else environment.AI_ALLOW_CUSTOM_BASE_URL = originalCustomBaseUrl;
  }
});
...
test("a browser-selected AI endpoint can only use its own validated session key", () => {
  ...
  const names = [
    ...
    "AI_API_KEY",
    "OPENAI_BASE_URL",
    "AI_ALLOW_UNAUTHENTICATED_SERVER_KEY",
    "AI_ALLOW_CUSTOM_BASE_URL",
  ] as const;
  const original = Object.fromEntries(names.map((name) => [name, environment[name]])) as Record<
    (typeof names)[number],
    string | undefined
  >;
  try {
...
test("structured document analysis rejects malformed model output", () => {
  assert.throws(
    () => parseDocumentAnalysis(JSON.stringify({ documentTitle: "缺字段" })),
    /格式无效/,
  );
});

test("course synthesis is parsed only when all required evidence fields exist", () => {
  const parsed = parseCourseSynthesis(
    JSON.stringify({
...
test("AI plan output rejects malformed, oversized or empty plans", () => {
  ...
  assert.throws(
    () => parseStudyPlan(JSON.stringify({ plan: [{ ...valid, durationMinutes: 0 }] })),
    /时长无效/,
  );
  assert.throws(
    () => parseStudyPlan(JSON.stringify({ plan: [{ ...valid, type: "闲聊" }] })),
    /格式无效/,
  );
  assert.throws(
    () => parseStudyPlan(JSON.stringify({ plan: [{ ...valid, date: "2026-02-30" }] })),
    /日期无效/,
  );
  assert.throws(
    () => parseStudyPlan(JSON.stringify({ plan: Array.from({ length: 36 }, () => valid) })),
    /数量过多/,
  );
});
