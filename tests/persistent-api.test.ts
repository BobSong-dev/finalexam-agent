  ...
  keyPoints: [
    {
      ...
      title: "极限",
      importance: 5,
      evidence: { label: "mock.pdf", location: "第 1 页", quote: "求极限" },
    },
    {
      id: "duplicate",
      title: "导数",
      importance: 4,
      evidence: { label: "mock.pdf", location: "第 1 页", quote: "求导数" },
    },
  ],
  questionPatterns: [],
  studyActions: ["完成练习。"],
...
test("persistent API validates inputs, hides storage internals, and invalidates stale synthesis", async () => {
  ...
  try {
    const malformedCourse = await createCourse(
    ...
    for (const field of [
      ...
      const invalidTypeResponse = await patchWorkspace(
      ...
      assert.equal(
        invalidTypeResponse.status,
        400,
        `${field} must reject an explicitly supplied non-string value`,
      );
      assert.match(await invalidTypeResponse.text(), new RegExp(field));
    }
    const afterTypeErrors = await readWorkspace();
    assert.equal(
      afterTypeErrors.updatedAt,
      beforeTypeErrors.updatedAt,
      "profile type errors must be rejected before storage is mutated",
    );
    ...
    const invalidCombinedPatch = await patchWorkspace(
      new NextRequest("http://localhost/api/workspace", {
        method: "PATCH",
    ...
    assert.equal(
      afterInvalidPatch.profile.displayName,
      beforeInvalidPatch.profile.displayName,
      "invalid availability must roll back the profile half of a combined PATCH",
    );
    assert.equal(
      afterInvalidPatch.updatedAt,
      beforeInvalidPatch.updatedAt,
      "a rejected combined PATCH must not write workspace state",
    );
    assert.equal(afterInvalidPatch.auditLog.length, beforeInvalidPatch.auditLog.length);

    const profileAuditCount = afterInvalidPatch.auditLog.filter(
      (event) => event.action === "profile.updated",
    ).length;
    const availabilityAuditCount = afterInvalidPatch.auditLog.filter(
      (event) => event.action === "availability.updated",
    ).length;
    ...
    const availabilityOnlyPatch = await patchWorkspace(
      new NextRequest("http://localhost/api/workspace", {
        method: "PATCH",
    ...
    assert.equal(
      afterAvailabilityPatch.auditLog.filter((event) => event.action === "profile.updated").length,
      profileAuditCount,
      "availability-only PATCH must not emit a false profile audit",
    );
    assert.equal(
      afterAvailabilityPatch.auditLog.filter((event) => event.action === "availability.updated")
        .length,
      availabilityAuditCount + 1,
    );
    ...
    form.append(
      "file",
      new File([Buffer.from("%PDF-1.4 mock")], "notes.pdf", { type: "text/html" }),
    );
    ...
    await assert.rejects(
      () => beginMaterialAnalysis(materialId),
      (error: unknown) => {
        return Boolean(
    ...
    let workspace = await saveDocumentAnalysis(
      materialId,
      documentAnalysis,
      analysisReservation.runId,
    );
    const materialInsightIds = workspace.insights
      .filter((item) => item.courseId === coursePayload.course.id)
      .map((item) => item.id);
    assert.equal(
      new Set(materialInsightIds).size,
      materialInsightIds.length,
      "untrusted model IDs cannot collide in persisted UI IDs",
    );
    const wrongChoice = await recordPractice(coursePayload.course.id, {
      [`material-${materialId}-question-0`]: "not-a",
    });
    assert.equal(
      wrongChoice.score,
      0,
      "an arbitrary answer containing A must not pass an A multiple-choice question",
    );

    workspace = await saveCourseSynthesis(
      coursePayload.course.id,
      synthesis,
      createCourseSynthesisSourceSnapshot(workspace, coursePayload.course.id),
    );
    ...
    assert.equal(
      workspace.courseSyntheses[coursePayload.course.id],
      undefined,
      "new document analysis invalidates an old course synthesis",
    );
    assert.equal(
      workspace.insights.some((item) =>
        item.id.startsWith(`synthesis-${coursePayload.course.id}-`),
      ),
      false,
    );
    ...
    const exportResponse = await exportWorkspace(
      new NextRequest("http://localhost/api/workspace/export"),
    );
    assert.equal(exportResponse.status, 200);
    assert.match(exportResponse.headers.get("content-disposition") ?? "", /attachment/);
    const exportBody = (await exportResponse.json()) as {
      materials: Array<{ id: string; objectKey?: string; sha256?: string }>;
    };
    assert.equal(
      typeof exportBody.materials.find((material) => material.id === materialId)?.objectKey,
      "string",
    );
    ...
    const invalidAssessment = await submitAssessment(
