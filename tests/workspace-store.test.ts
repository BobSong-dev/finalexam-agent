...
const temporaryDataDirectory = path.join(
  os.tmpdir(),
  `finale-workspace-store-${process.pid}-${randomUUID()}`,
);
...
test("self-hosted workspace persists the full material-to-practice flow", async () => {
  ...
  Object.defineProperty(streamedUpload, "arrayBuffer", {
    value: async () => {
      throw new Error("persisted uploads must stream instead of buffering the whole file");
    },
  });
  ...
  const storedJson = JSON.parse(
    await readFile(path.join(temporaryDataDirectory, "workspace.json"), "utf8"),
  ) as {
  ...
  await assert.rejects(
    () => readFile(orphanObject),
    (error: unknown) =>
      Boolean(error && typeof error === "object" && "code" in error && error.code === "ENOENT"),
  );
  await assert.rejects(
    () => readFile(interruptedUpload),
    (error: unknown) =>
      Boolean(error && typeof error === "object" && "code" in error && error.code === "ENOENT"),
  );
  assert.equal(
    (await readFile(path.join(uploadsDirectory, material.objectKey))).toString("utf8"),
    sourceText,
  );
  ...
  workspace = await saveDocumentAnalysis(
    material.id,
    mockedDocumentAnalysis,
    analysisReservation.runId,
  );
  assert.equal(workspace.materials[0]?.status, "已分析");
  assert.equal(workspace.materials[0]?.pages, 3);
  assert.deepEqual(
    workspace.documentAnalyses[material.id]?.keyPoints.map((point) => point.title),
    ["二重积分的区域变换"],
  );
  assert.equal(
    workspace.insights.some(
      (item) =>
        item.id.startsWith(`material-${material.id}-`) && item.title === "二重积分的区域变换",
    ),
    true,
  );
  assert.equal(
    workspace.questions.some(
      (question) => question.id.startsWith(`material-${material.id}-`) && question.answer === "A",
    ),
    true,
  );

  workspace = await saveCourseSynthesis(
    course.id,
    mockedCourseSynthesis,
    createCourseSynthesisSourceSnapshot(workspace, course.id),
  );
  assert.equal(workspace.courseSyntheses[course.id]?.summary, mockedCourseSynthesis.summary);
  assert.equal(
    workspace.insights.some(
      (item) =>
        item.id.startsWith(`synthesis-${course.id}-`) && item.title === "二重积分的区域变换",
    ),
    true,
  );
  assert.equal(
    workspace.insights
      ...
      .every((item) => item.id.startsWith(`synthesis-${course.id}-`)),
    true,
  );
  assert.equal(
    workspace.questions.some(
      (question) => question.id.startsWith(`synthesis-${course.id}-`) && question.answer === "区域",
    ),
    true,
  );
  ...
  // Three perfect rounds lift the knowledge point to ≥80, which schedules the
  // light confirmation action instead of the first-pass study action that was
  // completed above.
  assert.equal(
    replannedTask?.title.includes("快速回顾「二重积分的区域变换」"),
    true,
    replannedTask?.title,
  );
  assert.equal(
    replannedTask?.status,
    "待完成",
    "a changed task must not inherit a stale completion",
  );

  const finalWorkspace = await getWorkspace();
  assert.equal(finalWorkspace.materials[0]?.status, "已分析");
  assert.equal(
    finalWorkspace.courseSyntheses[course.id]?.highFrequencyPoints[0]?.title,
    "二重积分的区域变换",
  );
...
test("a stuck analysis reservation becomes retryable after the staleness window", async () => {
  ...
  const { material } = await storeUploadedMaterial(
    course.id,
    makeStudyFile("操作系统模拟卷.pdf", "%PDF-mock\nstale analysis material"),
  );
  ...
  const persisted = JSON.parse(await readFile(statePath, "utf8")) as {
    materials: Array<{
      id: string;
      updatedAt: string;
      analysisLease?: { runId: string; startedAt: string };
    }>;
  };
  ...
  assert.equal(
    resumed.workspace.materials.find((item) => item.id === material.id)?.status,
    "分析中",
  );

  const publicMaterial = toPublicMaterial(
    resumed.workspace.materials.find((item) => item.id === material.id)!,
  );
  const publicWorkspace = toPublicWorkspace(resumed.workspace);
  assert.equal(
    "analysisLease" in publicMaterial,
    false,
    "analysis ownership tokens are server-only",
  );
  assert.equal(
    JSON.stringify(publicWorkspace).includes(resumed.runId),
    false,
    "public workspace output must not contain an analysis run id",
  );
  ...
  assert.equal(
    active.materials.find((item) => item.id === material.id)?.analysisLease?.runId,
    resumed.runId,
  );
  ...
  assert.equal(
    completed.materials.find((item) => item.id === material.id)?.analysisLease,
    undefined,
  );
});
...
test("AI plan commits reject superseded runs and changed generation inputs", async () => {
  ...
  assert.equal(
    firstReservation.inputHash,
    secondReservation.inputHash,
    "identical inputs should have the same evidence hash",
  );
  assert.notEqual(
    firstReservation.runId,
    secondReservation.runId,
    "each run still needs unique ownership",
  );
  ...
  assert.equal(
    afterNewestCommit.tasks.some((task) => task.id === tasks[0]!.id),
    true,
  );
  ...
  assert.equal(
    "planGenerationLease" in publicState,
    false,
    "plan ownership tokens are server-only",
  );
  assert.equal(JSON.stringify(publicState).includes(staleReservation.runId), false);

  await updateAvailability(
    staleReservation.context.availability.map((day, index) => ({
      ...day,
...
test("availability contract matches the persisted 7-day plan window", async () => {
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof WorkspaceStoreError && error.status === 400 && /1–7 天/.test(error.message),
...
test("download filenames are sanitized without mangling ordinary letters", () => {
  ...
  assert.equal(
    sanitizeDownloadFilename('a/b\\c:d*e?f"g<h>i|j\r\nk.pdf'),
    "a_b_c_d_e_f_g_h_i_j_k.pdf",
  );
...
test("dates are computed in the learner timezone regardless of the host timezone", () => {
  ...
  assert.match(
    dateOnlyInTimeZone("Not/AZone", instant),
    /^\d{4}-\d{2}-\d{2}$/,
    "invalid zones fall back to the host date without throwing",
  );
});
...
test("uncompleted past-day tasks become missed records instead of vanishing", async () => {
  ...
  assert.equal(
    rebuilt.tasks.some((task) => task.id === "missed-task-test"),
    false,
    "past-day tasks leave the active plan",
  );
  ...
  assert.equal(
    again.missedTasks.filter((task) => task.id === "missed-task-test").length,
    1,
    "missed records are deduplicated on later rebuilds",
  );
});
...
test("profile study day start shifts every generated task time", async () => {
  ...
  assert.ok(
    workspace.tasks.some((task) => task.start === "09:00"),
    "the first slot of the day starts at the configured time",
  );
  assert.ok(workspace.tasks.every((task) => task.start >= "09:00"));
  assert.equal(
    workspace.tasks.some((task) => task.start === "18:30"),
    false,
    "the previous default start must not linger",
  );
  await assert.rejects(
...
test("fill-in answers require an exact match and partial submissions are not 100", async () => {
  ...
  const { material } = await storeUploadedMaterial(
    course.id,
    makeStudyFile("线代模拟.pdf", "%PDF-mock\nscoring"),
  );
  ...
  const partial = await recordPractice(course.id, {
    [questionIds[0]!]: workspace.questions.find((question) => question.id === questionIds[0])!
      .answer,
  });
