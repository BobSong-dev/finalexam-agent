...
import {
  WorkspaceStoreError,
  archiveExpiredSharedMaterials,
  beginMaterialAnalysis,
  createCourse,
  contributeSharedMaterial,
  getWorkspace,
  listCommunityMaterials,
  listModerationQueue,
  reportSharedMaterial,
  ...
  keyPoints: [
    {
      ...
      title: "极限",
      importance: 4,
      evidence: { label: "community.pdf", location: "第 1 页", quote: "求极限" },
    },
  ],
  questionPatterns: [],
  studyActions: ["完成练习"],
...
test("email verification and community moderation persist honest local state", async () => {
  ...
  const old = {
    data: process.env.FINALE_DATA_DIR,
    secret: process.env.AUTH_OTP_SECRET,
    mail: process.env.EMAIL_PROVIDER_URL,
    domains: process.env.SCHOOL_EMAIL_DOMAINS,
    admin: process.env.COMMUNITY_ADMIN_TOKEN,
  };
  let sentCode = "";
  let deliveryCount = 0;
  ...
  const provider = createServer((request, response) => {
    ...
    request.on("end", () => {
      try {
        sentCode = JSON.parse(body).text.match(/\d{6}/)?.[0] ?? "";
      } catch {
        sentCode = "";
      }
      deliveryCount += 1;
  ...
  try {
    ...
    const uploaded = await storeUploadedMaterial(
      course.id,
      new File([uploadedBytes], "community.pdf", { type: "application/pdf" }),
    );
    ...
    for (let attempt = 1; attempt <= 5; attempt += 1) {
      const rejected = await verifyEmailOtp("locked@example.edu", wrongLockedCode).catch(
        (error: unknown) => error,
      );
      ...
      assert.equal(
        (await getWorkspace()).otpChallenges.find(
          (challenge) => challenge.email === "locked@example.edu",
        )?.attempts,
        attempt,
      );
    }
    const exhausted = await verifyEmailOtp("locked@example.edu", lockedCode).catch(
      (error: unknown) => error,
    );
    ...
    const duplicate = concurrent.find(
    ...
    assert.equal(
      deliveryCount,
      beforeConcurrentDelivery + 1,
      "concurrent requests for one email must deliver exactly one message",
    );

    const beforeFailedDelivery = deliveryCount;
    failNextDelivery = true;
    const failedDelivery = await requestEmailOtp("retry@example.edu").catch(
      (error: unknown) => error,
    );
    ...
    assert.equal(
      retriedDelivery.email,
      "retry@example.edu",
      "a provider failure must release the in-flight guard for immediate retry",
    );
    assert.equal(deliveryCount, beforeFailedDelivery + 2);

    const submitted = await contributeSharedMaterial({
      materialId: uploaded.material.id,
      consent: true,
    ...
    const moderationResponse = await moderateRoute(
      new NextRequest("http://localhost/api/shared/moderate", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: "Bearer test-admin-token" },
        body: JSON.stringify({
          materialId: submitted.material.id,
          decision: "approve",
          quality: "已核验",
        }),
      }),
    ...
    const moderationPayload = (await moderationResponse.json()) as {
      ...
      workspace?: { profile?: { credits?: number } };
    };
    ...
    assert.equal(
      ((await missingDownload.json()) as { error?: string }).error,
      "未找到共享资料。",
      "trusted community errors retain their public message and status",
    );
    ...
    assert.deepEqual(
      Buffer.from(await sharedDownload.arrayBuffer()),
      uploadedBytes,
      "the zero-copy response view must preserve every shared-file byte",
    );

    const sharedRecord = (await getWorkspace()).sharedMaterialRecords.find(
      (record) => record.id === submitted.material.id,
    );
    ...
    try {
      ...
      assert.doesNotMatch(
        ...
        new RegExp(sharedRecord.objectKey.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
        "unknown filesystem errors must not expose storage details",
      );
    } finally {
    ...
    assert.equal(
      await readFile(catalogStatePath, "utf8"),
      beforeCatalogRead,
      "a catalog read with nothing to archive must not rewrite workspace state",
    );
    ...
    const queueResponse = await queueRoute(
      new NextRequest("http://localhost/api/shared/moderation/queue"),
    );
    assert.equal(queueResponse.status, 403, "the queue must require the admin bearer credential");
    const authorizedQueue = await queueRoute(
      new NextRequest("http://localhost/api/shared/moderation/queue", {
    ...
    const persisted = JSON.parse(await readFile(statePath, "utf8")) as {
      sharedMaterialRecords: Array<{ accessEndsOn: string }>;
    };
    ...
    if (old.data === undefined) delete process.env.FINALE_DATA_DIR;
    else process.env.FINALE_DATA_DIR = old.data;
    if (old.secret === undefined) delete process.env.AUTH_OTP_SECRET;
    else process.env.AUTH_OTP_SECRET = old.secret;
    if (old.mail === undefined) delete process.env.EMAIL_PROVIDER_URL;
    else process.env.EMAIL_PROVIDER_URL = old.mail;
    if (old.domains === undefined) delete process.env.SCHOOL_EMAIL_DOMAINS;
    else process.env.SCHOOL_EMAIL_DOMAINS = old.domains;
    if (old.admin === undefined) delete process.env.COMMUNITY_ADMIN_TOKEN;
    else process.env.COMMUNITY_ADMIN_TOKEN = old.admin;
