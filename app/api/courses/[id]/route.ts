...
export async function PATCH(request: NextRequest, context: RouteContext<"/api/courses/[id]">) {
  try {
    ...
    if (!isCoursePatch(body))
      return NextResponse.json({ error: "课程更新请求必须是 JSON 对象。" }, { status: 400 });
...
export async function DELETE(request: NextRequest, context: RouteContext<"/api/courses/[id]">) {
  try {
    ...
    const { id } = await context.params;
    return NextResponse.json(toPublicWorkspace(await deleteCourse(id)), {
      headers: { "Cache-Control": "no-store" },
    });
  } catch (error) {
    const security = securityErrorResponse(error);
...
function isCoursePatch(value: unknown): value is CoursePatch {
  ...
  return (
    entries.length > 0 &&
    entries.every(([key, item]) => allowed.has(key) && typeof item === "string")
  );
}
