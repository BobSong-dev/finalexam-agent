...
export async function POST(request: NextRequest, context: DynamicRouteContext) {
  ...
  try {
    ...
    if (!analyses.length)
      throw new WorkspaceStoreError("请先完成至少一份资料的 AI 分析，再生成课程综合。", 400);
    ...
    return NextResponse.json(
      {
        ...
        model,
        analysis: synthesis,
        workspace: toPublicWorkspace(nextWorkspace),
      },
    ...
    return NextResponse.json(
      { error: "课程综合无法完成，请检查 AI 配置和网络后重试。" },
      { status: 500 },
    );
  } finally {
    releaseCapacity?.();
