...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(toPublicWorkspace(await getWorkspace()), {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!isRecord(body))
      return NextResponse.json({ error: "工作区更新必须是 JSON 对象。" }, { status: 400 });
    const invalidStringField = profileStringFields.find(
      (field) =>
        Object.prototype.hasOwnProperty.call(body, field) && typeof body[field] !== "string",
    );
    if (invalidStringField)
      return NextResponse.json({ error: `${invalidStringField} 必须是字符串。` }, { status: 400 });
