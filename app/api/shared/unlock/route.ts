...
export async function POST(request: NextRequest) {
  try {
    ...
    if (typeof materialId !== "string" || !materialId.trim())
      return NextResponse.json({ error: "materialId 为必填项。" }, { status: 400 });
    const result = await unlockSharedMaterial(materialId.trim());
    return NextResponse.json(
      { unlocked: true, material: result.material, workspace: toPublicWorkspace(result.workspace) },
