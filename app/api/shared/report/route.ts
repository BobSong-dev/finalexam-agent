...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body))
      return NextResponse.json({ error: "举报请求必须是 JSON 对象。" }, { status: 400 });
    ...
    if (
      ...
      return NextResponse.json({ error: "materialId、reason 为必填项。" }, { status: 400 });
    ...
    return NextResponse.json(
      { accepted: true, notice: "举报已记录，管理员会在审核队列中处理。" },
