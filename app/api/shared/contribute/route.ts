...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body))
      return NextResponse.json({ error: "共享请求必须是 JSON 对象。" }, { status: 400 });
    ...
    if (
      ...
      typeof payload.privacyConfirmed !== "boolean"
    )
      return NextResponse.json(
        { error: "materialId、consent 和 privacyConfirmed 为必填项。" },
        { status: 400 },
      );
    const result = await contributeSharedMaterial({
    ...
    return NextResponse.json(
      {
        ...
        notice: "资料已提交审核；审核通过前不会出现在可解锁目录，也不会发放积分。",
      },
