...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body))
      return NextResponse.json({ error: "审核请求必须是 JSON 对象。" }, { status: 400 });
    ...
    if (
      ...
      return NextResponse.json({ error: "materialId 和 decision 为必填项。" }, { status: 400 });
    if (
      payload.quality !== undefined &&
      !["优质", "已核验", "待核验"].includes(String(payload.quality))
    )
      return NextResponse.json({ error: "quality 无效。" }, { status: 400 });
    const result = await moderateSharedMaterial({
      materialId: payload.materialId.trim(),
      decision: payload.decision,
      quality: payload.quality as "优质" | "已核验" | "待核验" | undefined,
      reason: typeof payload.reason === "string" ? payload.reason : undefined,
    });
    return NextResponse.json(
      { material: result.material, workspace: toPublicWorkspace(result.workspace) },
