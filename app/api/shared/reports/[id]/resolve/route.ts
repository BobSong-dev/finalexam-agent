...
export async function POST(
  request: NextRequest,
  context: RouteContext<"/api/shared/reports/[id]/resolve">,
) {
  try {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body))
      return NextResponse.json({ error: "处理请求必须是 JSON 对象。" }, { status: 400 });
    const resolution = (body as { resolution?: unknown }).resolution;
    if (typeof resolution !== "string" || !resolution.trim())
      return NextResponse.json({ error: "resolution 为必填项。" }, { status: 400 });
    await resolveSharedReport(id, resolution);
    return NextResponse.json(
      { accepted: true, notice: "举报已标记为已处理。" },
