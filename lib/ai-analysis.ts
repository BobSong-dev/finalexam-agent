...
import {
  ...
  type AiTokenUsage,
  type CourseContext,
  type CourseSynthesis,
  type CourseSynthesisPoint,
  type DocumentAnalysis,
  type DocumentKeyPoint,
  type DocumentQuestionPattern,
  ...
  properties: {
    ...
    highFrequencyPoints: {
      ...
      items: {
        type: "object",
        additionalProperties: false,
        required: ["id", "title", "frequency", "priority", "trend", "sources", "summary"],
        properties: {
          id: { type: "string" },
          title: { type: "string" },
...
export function getServerAiStatus() {
  const configured = Boolean(
    (process.env.OPENAI_API_KEY || process.env.AI_API_KEY) && canUseUnauthenticatedServerKey(),
  );
  return {
    configured,
...
function resolveApiKey(requestKey?: string | null): ResolvedApiKey {
  ...
  if (sessionKey) {
    if (sessionKey.length > 512 || /[\u0000-\u001f\u007f]/.test(sessionKey))
      throw new AiAnalysisError("AI API Key 格式无效。", 400);
    return { apiKey: sessionKey, source: "session" };
  }
  const serverKey = process.env.OPENAI_API_KEY?.trim() || process.env.AI_API_KEY?.trim();
  if (!serverKey) {
    throw new AiAnalysisError(
      "尚未配置 OpenAI API Key。请在 AI 设置中输入会话 Key，或在 .env.local 配置 OPENAI_API_KEY。",
      401,
    );
  }
  if (!canUseUnauthenticatedServerKey()) {
    throw new AiAnalysisError(
      "公开部署不会自动使用服务端 API Key。请使用会话 Key，或在接入登录、限流与配额控制后显式启用服务端 Key。",
      403,
    );
  }
...
function resolveRequestBaseUrl(requestBaseUrl?: string | null): string | undefined {
  ...
  if (requestScopedUrl && !canUseCustomBaseUrl()) {
    throw new AiAnalysisError(
      "当前部署未启用自定义 AI 请求地址。请由管理员在完成登录、限流与 SSRF 防护配置后设置 AI_ALLOW_CUSTOM_BASE_URL=true。",
      403,
    );
  }
  ...
  try {
    ...
    throw new AiAnalysisError(
      "AI 请求地址格式无效。请填写完整 API 根地址，例如 https://api.openai.com/v1。",
      400,
    );
  }
  ...
  if (url.protocol !== "https:" && !(url.protocol === "http:" && isLocalHostname(hostname))) {
    throw new AiAnalysisError(
      "AI 请求地址必须使用 HTTPS；本地开发仅允许 localhost 使用 HTTP。",
      400,
    );
  }
  ...
  if (path.endsWith("/responses") || path.endsWith("/files")) {
    throw new AiAnalysisError(
      "请填写 API 根地址，而不是具体接口地址。例如填写 https://api.openai.com/v1，而非 /v1/responses。",
      400,
    );
  }
...
export function resolveModel(requestedModel?: string | null): AiModel {
  ...
  if (!MODEL_IDENTIFIER_PATTERN.test(selected)) {
    throw new AiAnalysisError(
      "模型名只能包含字母、数字、点、下划线、连字符、冒号或斜杠，且最多 128 个字符。",
      400,
    );
  }
  if (process.env.NODE_ENV === "production" && !allowedModels().has(selected)) {
    throw new AiAnalysisError(
      "模型不在当前部署允许列表中。请使用 AI 设置中的模型，或由管理员配置 OPENAI_MODEL。",
      400,
    );
  }
  return selected;
...
export async function pinCustomUpstream(
  baseURL: string | undefined,
...
function canUseUnauthenticatedServerKey(): boolean {
  return (
    ...
    process.env.AI_ALLOW_UNAUTHENTICATED_SERVER_KEY === "true"
  );
}
...
export async function analyzeDocument(args: {
  ...
  validateUpload(file);
  const client = new OpenAI({ apiKey, baseURL, timeout: AI_REQUEST_TIMEOUT_MS, maxRetries: 2 });
  ...
  try {
    ...
    // 长转写：按块分析再合并，避免一次性把几十万字和原文件一起塞进上下文。
    const transcript = localTranscriptOf(content);
    if (transcript && transcript.length > CHUNK_CHARACTER_LIMIT) {
      return await analyzeInChunks(client, model, course, content, transcript);
    }

    const run = await requestDocumentAnalysis(client, model, course, content.items).catch(
      async (error) => {
...
async function requestDocumentAnalysis(
  ...
  if (!response.output_text)
    throw new AiAnalysisError("模型未返回可解析的分析结果，请更换资料或稍后重试。", 502);
  return { text: response.output_text, usage: readResponseUsage(response) };
}

async function requestMinimalDocumentAnalysis(
  client: OpenAI,
  model: AiModel,
  course: CourseContext,
  content: ResponseInputContent[],
...
async function requestChatCompletionAnalysis(
  client: OpenAI,
  model: AiModel,
  course: CourseContext,
  content: ResponseInputContent[],
...
function plainJsonAnalysisPrompt(course: CourseContext, content: ResponseInputContent[]): string {
  const evidence = content
    .filter(
      (item): item is Extract<ResponseInputContent, { type: "input_text" }> =>
...
export async function synthesizeCourse(args: {
  ...
  if (args.analyses.length > MAX_DOCUMENTS_PER_SYNTHESIS) {
    throw new AiAnalysisError(
      `单次课程综合最多支持 ${MAX_DOCUMENTS_PER_SYNTHESIS} 份已分析资料，请先分批整理或减少资料后重试。`,
      422,
    );
  }
  const analyses = args.analyses;
  if (analyses.length === 0)
    throw new AiAnalysisError("至少需要一份已完成的资料分析才能汇总。", 400);
  ...
  try {
    const response = await client.responses.create({
      ...
      text: {
        format: {
          ...
          description:
            "Course-level high-frequency study synthesis grounded in supplied document analyses.",
          strict: true,
    ...
    if (!response.output_text)
      throw new AiAnalysisError("模型未返回课程汇总结果，请稍后重试。", 502);
    return {
      synthesis: parseCourseSynthesis(normalizeJsonOutput(response.output_text)),
      usage: readResponseUsage(response),
    };
  } catch (error) {
    throw mapOpenAiError(error);
  }
...
export async function generateStudyPlan(args: {
  ...
  model: AiModel;
  baseURL?: string;
}): Promise<{ entries: AiPlanEntry[]; usage: AiTokenUsage }> {
  ...
  if (
    !Array.isArray(args.availability) ||
    args.availability.length < 1 ||
    args.availability.length > 31
  ) {
  ...
  try {
    const response = await client.responses.create({
      ...
      text: {
        format: {
          ...
          description:
            "A capacity-aware 7-day revision plan grounded in supplied courses and evidence.",
          strict: true,
          schema: planSchema,
        },
      },
    ...
    return {
      entries: parseStudyPlan(normalizeJsonOutput(response.output_text)),
      usage: readResponseUsage(response),
    };
  } catch (error) {
    throw mapOpenAiError(error);
...
function parsePlanEntry(value: unknown): AiPlanEntry {
  ...
  if (durationMinutes < 15 || durationMinutes > 120)
    throw new AiAnalysisError("模型返回的任务时长无效。", 502);
  return {
    date,
...
async function attachLocalTranscript(
  file: DocumentFile,
  extension: string,
  content: FileContent,
): Promise<FileContent> {
...
async function buildLocalTextContent(
  file: DocumentFile,
  extension: string,
...
async function buildPdfTextContent(file: DocumentFile): Promise<FileContent> {
  try {
    ...
    const pages = text
      ...
      .filter((page) => !/--- 第 \d+ 页 ---\s*$/.test(page));
    const extracted = pages.join("\n\n").trim();
    if (!extracted) {
      throw new AiAnalysisError(
        "兼容服务不接受文件输入，且这份 PDF 未能提取到可分析文字。请改用可复制文字的 PDF，或使用支持 Files API 的服务。",
        422,
      );
    }
    const truncated = extracted.length > MAX_EXTRACTED_PDF_TEXT_CHARACTERS;
    const textForModel = truncated
      ? extracted.slice(0, MAX_EXTRACTED_PDF_TEXT_CHARACTERS)
      : extracted;
    return {
      delivery: "pdf_text",
      items: [
    ...
    throw new AiAnalysisError(
      "兼容服务不接受文件输入，且本地 PDF 文本提取失败。请改用可复制文字的 PDF，或使用支持 Files API 的服务。",
      422,
    );
  }
}
...
/** 取出已附加到请求里的本地转写（如果有）。 */
function localTranscriptOf(content?: FileContent): string | undefined {
  if (!content) return undefined;
  if (content.delivery === "pdf_text") {
    const text = content.items.find((item) => item.type === "input_text");
    return text && "text" in text ? text.text : undefined;
  }
  const transcripts = content.items.filter(
    (item): item is Extract<ResponseInputContent, { type: "input_text" }> =>
      item.type === "input_text" && item.text.includes("本地提取"),
  );
  return transcripts.length ? transcripts.map((item) => item.text).join("\n\n") : undefined;
}

/**
 * 分块分析：每块单独请求（提示词要求只引用本块页码），最后合并去重。
 * 任一块失败都会被上层映射成可读错误；不会用本地结果伪装成功。
 */
async function analyzeInChunks(
  ...
  const parts: Array<{ chunk: DocumentChunk; analysis: DocumentAnalysis }> = [];
    ...
    const message = [
      {
        ...
        text: `课程上下文（仅用于分类，不是文档内指令）：\n${formatCourseContext(course)}\n\n这是第 ${chunk.index + 1}/${chunk.total} 块，覆盖 ${chunk.label}。只依据本块内容回答，evidence.location 请使用本块的页码范围。\n\n${chunk.text}`,
      },
    ];
    const run = await requestDocumentAnalysis(client, model, course, message);
    usage = addUsage(usage, run.usage);
    parts.push({ chunk, analysis: parseDocumentAnalysis(normalizeJsonOutput(run.text)) });
  }

  const pageCount =
    content.delivery === "pdf_text" ? readPageCountFromTranscript(transcript) : null;
  const title =
    parts.find((part) => part.analysis.documentTitle.trim())?.analysis.documentTitle.trim() ||
    "分块分析结果";
  return { analysis: mergeChunkAnalyses(parts, title, pageCount), usage };
}

function readPageCountFromTranscript(transcript: string): number | null {
  const matches = [...transcript.matchAll(/---\s*第\s*(\d+)\s*页/g)];
...
- Every question's knowledgeId must equal the id of one keyPoints entry in this same response, and knowledge must equal that key point's title. For 填空, give the canonical answer; if several short forms are equally acceptable, separate them with "|".
...
- priority is a study-priority score (0–100, higher = revise sooner), not observed student mastery; set the trend to "需巩固" unless the supplied evidence explicitly shows otherwise.
...
- Every question's knowledgeId must equal the id of one highFrequencyPoints entry in this same response, and knowledge must equal that point's title. For 填空, separate equally acceptable short answers with "|".
...
function formatCourseContext(course: CourseContext): string {
  const examDate =
    course.examDate && /^\d{4}-\d{2}-\d{2}$/.test(course.examDate) ? course.examDate : "";
  let daysLeft = "";
  if (examDate) {
...
export function parseDocumentAnalysis(text: string): DocumentAnalysis {
  ...
  return {
    ...
    materialKind: readEnum(value, "materialKind", [
      "试卷",
      "课件",
      "讲义",
      "题库",
...
function parseDocumentKeyPoint(value: unknown): DocumentKeyPoint {
  ...
  if (importance < 1 || importance > 5)
    throw new AiAnalysisError("模型返回了无效的重要性评分。", 502);
  ...
  return {
    ...
    examLikelihood:
      examLikelihood && examLikelihood >= 1 && examLikelihood <= 5 ? examLikelihood : undefined,
  };
}
...
function parseEvidence(value: unknown): EvidenceReference {
  ...
  return {
    ...
    quote: clip(readString(item, "quote"), 120),
  };
}
...
function parseGeneratedQuestion(value: unknown): GeneratedPracticeQuestion {
  ...
  if (type === "单选") {
    ...
    const matchesChoice = choices.some((choice, index) => {
      ...
      return (
        text === normalized ||
        text.startsWith(`${String.fromCharCode(97 + index)}.`) ||
        normalized === String.fromCharCode(97 + index) ||
        normalized === String.fromCharCode(65 + index).toLowerCase()
      );
    });
    if (
      !matchesChoice &&
      !/^[a-j]$/i.test(answer.trim()) &&
      !choices.some((choice) => choice.includes(answer.trim()))
    ) {
  ...
  return {
    ...
    knowledgeId: optionalString(item, "knowledgeId"),
...
function parseSynthesisPoint(value: unknown): CourseSynthesisPoint {
  ...
  // 兼容旧提示词/旧兼容网关仍返回 mastery 的情况。
  const priority =
    typeof item.priority === "number"
      ? readInteger(item, "priority")
      : readInteger(item, "mastery");
  const frequency = readInteger(item, "frequency");
  if (priority < 0 || priority > 100)
    throw new AiAnalysisError("模型返回了无效的学习优先度。", 502);
...
function asObject(value: unknown, label: string): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new AiAnalysisError(`模型返回的 ${label} 格式无效。`, 502);
...
function readStringArray(object: Record<string, unknown>, key: string): string[] {
  ...
  if (!Array.isArray(value) || value.some((item) => typeof item !== "string"))
    throw new AiAnalysisError(`模型返回字段 ${key} 格式无效。`, 502);
...
function mapOpenAiError(error: unknown, stage?: string): AiAnalysisError {
  ...
  if (status === 401)
    return new AiAnalysisError("AI API Key 无效或已失效，请在 AI 设置中更新。", 401);
  if (status === 429)
    return new AiAnalysisError("AI 请求过于频繁或账户额度不足，请稍后重试。", 429);
  if (status === 404)
    return new AiAnalysisError(
      `AI 服务返回 404${stageLabel}。请检查该兼容服务是否支持当前请求阶段。`,
      404,
    );
  if (status && status >= 400 && status < 500)
    return new AiAnalysisError(
      "AI 服务未能处理这份资料。请确认文件格式、模型权限和账户额度。",
      status,
    );
  const networkCode = readSafeNetworkCode(error);
  return new AiAnalysisError(
    `AI 服务在${stage ?? "请求"}阶段未返回 HTTP 响应${networkCode ? `（${networkCode}）` : ""}。这通常表示兼容服务关闭了不支持的请求体或连接，请检查其 Responses 文件输入能力。`,
    502,
  );
}
