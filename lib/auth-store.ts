...
function otpSecret(): string {
  ...
  if (!secret || secret.length < 32)
    throw new AuthError(
      "管理员尚未配置 AUTH_OTP_SECRET，邮箱验证暂不可用。",
      503,
      "AUTH_NOT_CONFIGURED",
    );
  return secret;
...
function schoolForEmail(email: string): string | undefined {
  ...
  const mappings = (process.env.SCHOOL_EMAIL_DOMAINS ?? "")
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
  for (const mapping of mappings) {
    const [configuredDomain, school] = mapping.split("=", 2).map((item) => item?.trim());
    if (configuredDomain && school && configuredDomain.toLowerCase() === domain)
...
function providerUrl(): string {
  ...
  if (!value)
    throw new AuthError(
      "管理员尚未配置 EMAIL_PROVIDER_URL，邮箱验证暂不可用。",
      503,
  ...
  try {
    ...
    throw new AuthError("EMAIL_PROVIDER_URL 配置无效。", 503, "EMAIL_PROVIDER_NOT_CONFIGURED");
  }
  if (
    url.protocol !== "https:" &&
    !(
    ...
    throw new AuthError(
      "生产环境的 EMAIL_PROVIDER_URL 必须使用 HTTPS。",
      503,
      "EMAIL_PROVIDER_NOT_CONFIGURED",
    );
  }
...
export async function requestEmailOtp(
  ...
  if (otpRequestsInFlight.has(email))
    throw new AuthError("验证码已发送，请稍候再试。", 429, "OTP_COOLDOWN");
  ...
  try {
    ...
    const recent = current.otpChallenges.find(
      (challenge) =>
        challenge.email === email && Date.now() - Date.parse(challenge.sentAt) < OTP_COOLDOWN_MS,
    );
    ...
    if (process.env.NODE_ENV === "production" && !providerToken) {
      throw new AuthError(
        "管理员尚未配置 EMAIL_PROVIDER_TOKEN，邮箱验证暂不可用。",
        503,
        "EMAIL_PROVIDER_NOT_CONFIGURED",
      );
    }
...
export async function verifyEmailOtp(
  emailInput: unknown,
  codeInput: unknown,
): Promise<{ workspace: WorkspaceState; schoolMatched: boolean }> {
  const email = normalizeEmail(emailInput);
